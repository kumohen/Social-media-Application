    ...
    renderResultsHTML(posts){
      if(posts.length){
        ...
        ${posts.map(post =>{
          ...
          <span class="text-muted small">by ${post.author.user} on ${postDate.getMonth()}/${postDate.getDate()}/${postDate.getFullYear()}</span>
        </a>`
        }).join('')}
        
       </div>`)
      }else{
        this.resultsArea.innerHTML = `<p class="alert alert-danger text-center shadow-sm">we should not find any result</p>`
      }
      this.hideLoaderIcon()
